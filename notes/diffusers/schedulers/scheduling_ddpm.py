...
# Scheduler 역할 수행
# SchedulerMixin: .step() 등의 스케줄러 로직 포함하는 추상 클래스
# ConfigMixin: .config로 관리
# 한 step을 어떻게 계산할지를 정의
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if trained_betas is not None:
            # 노이즈 증가량
            # beta_schedule: linear 기본, variance_type은 sigma^2 계산 방식
        ...
        self.alphas = 1.0 - self.betas # 유지되는 신호의 비율
        self.alphas_cumprod = torch.cumprod(self.alphas, dim=0) # ᾱₜ = ∏₀^t αₖ, 현재까지 살아남은 정보 비율
    ...
    def scale_model_input(self, sample: torch.Tensor, timestep: Optional[int] = None) -> torch.Tensor:
        ...
        """
        return sample
# leading: 앞쪽 t만 뽑음 -> 빠른 추론
# linspace: 균등 간격
# trailing: 뒷쪽 t만 촘촘히
# → self.timesteps = tensor([999, 979, 959, ..., 39, 19, 0])
    def set_timesteps(
        self,
