...
class Encoder(nn.Module): #q(z|x), p(z|x)
    r"""
...
class Decoder(nn.Module): #p(z), p(x|z), p(x)
    r"""
    ...
    def forward(
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing: #grad_enabled를 모으고 loss에 최종 반영하는 것 같은데..
            # middle
...
class DiagonalGaussianDistribution(object):
    def __init__(self, parameters: torch.Tensor, deterministic: bool = False):
        ...
        #params를 dim index 기준으로 2개씩 나누겠다. 즉 (batch, 6, W, H) -> 2 * (batch, 3, W, H) 나눠서 각각 평균 분산 취급
        ...
        if self.deterministic: #결정론적이면 항상 평균만 반환하게 됨. 분산, 표준편차는 0임.
            self.var = self.std = torch.zeros_like( #0이 되도록 함 = 결정론적 (한 point에서 수렴해야 하기 때문)
                self.mean, device=self.parameters.device, dtype=self.parameters.dtype
            )
    #generator가 Optional[torch.Generator]형태여야 하며, 그렇지 못할 경우 None. 함수가 반환되는 것은 torch.Tensor 상태
    def sample(self, generator: Optional[torch.Generator] = None) -> torch.Tensor: #Noise
        # make sure sample is on the same device as the parameters and has same dtype
        sample = randn_tensor( #랜덤 샘플링
            self.mean.shape,
            generator=generator, #난수 생성 (가우시안 분포 따라야 함.)
            device=self.parameters.device,
            dtype=self.parameters.dtype,
        )
        #결정되는 Mean , std에 난수 Noise
        x = self.mean + self.std * sample
        return x

    def kl(self, other: "DiagonalGaussianDistribution" = None) -> torch.Tensor:
        if self.deterministic: #결정론에선 kl 차이나면 안되므로 0
            return torch.Tensor([0.0])
        else:
            if other is None:#prior 분포(표준정규분포)와 encoder posterior 비교 
                return 0.5 * torch.sum(
                    torch.pow(self.mean, 2) + self.var - 1.0 - self.logvar,
                    dim=[1, 2, 3],
                )
            else:
                return 0.5 * torch.sum( #두 가우시안 분포의 비교 (사전분포/사후분포)
